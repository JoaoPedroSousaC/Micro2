...
void Utility_Init(void);			//inicializa��o de fun��es dessa biblioteca
void Configure_Clock(void);			//configura��o do sistema de clock
void TIM2_Setup(void);				//ajusta o Timer2 para gera��o de base de tempo para delays
void Delay_Start(void);				//inicializa as fun��es de delay
void Delay_ms(uint32_t delay);		//delay em ms
void Delay_us(uint32_t delay);		//delay em us
void USART1_Init(void);				//configura��o e inicializa��o da USART1
void LCD_Init();                  	//inicializa��o do LCD com interface de 4 bits
void LCD_Clear();                 	//limpa o LCD
void LCD_SL();                    	//deslocamento � esquerda do LCD
void LCD_SR();                    	//deslocamento � direita do LCD
void LCD_CMD(unsigned char);      	//envia comando para o LCD
void LCD_DATA(unsigned char);     	//envia um dado de 4 bits para o LCD
void LCD_Set_Cursor(unsigned char, unsigned char);  //configura a posi��o do cursor
void LCD_Write_Char(char);        	//escreve um caracter no LCD na posi��o do cursor
void LCD_Write_String(char*);     	//escreve uma string no LCD a partir da posi��o do cursor

//Inicializa��o de fun��es dessa biblioteca
...
//Configura��o do sistema de clock para velocidade m�xima
...
//O timer 2 � o primeiro timer de uso geral de 32 bits.
//A configura��o abaixo usa o timer 2 como base para permitir o uso de delays em us e ms.
...
void USART1_Init(void)
{
	...
	USART1->BRR = 84;									//ajusta baud rate para 1 Mbps (frequ�ncia de clock de 84MHz)
...
{
   return (uint16_t)(USART1->DR & (uint16_t)0x01FF);	//l� o dado recebido
}
...
//Fun��es para manipula��o de LCDs
//Cria��o de estrutura de configura��o dos pinos do LCD
typedef struct
{
...
//inicializa��o da estrutura de configura��o dos pinos do LCD
...
void LCD_Set_Cursor(unsigned char linha, unsigned char coluna)
{
    ...
    if(linha == 1)
    {
    	Temp  = 0x80 + coluna - 1; //0x80 � o endere�o da primeira posi��o da linha 1
    	High4 = Temp >> 4;
    	Low4  = Temp & 0x0F;
    	LCD_CMD(High4);
    	LCD_CMD(Low4);
    ...
    if(linha == 2)
    {
    	Temp  = 0xC0 + coluna - 1;	//0xC0 � o endere�o da primeira posi��o da linha 2
    	High4 = Temp >> 4;
    	Low4  = Temp & 0x0F;
    	LCD_CMD(High4);
    	LCD_CMD(Low4);
    ...
    if(linha == 3)
    {
    	Temp  = 0x94 + coluna - 1;	//0x94 � o endere�o da primeira posi��o da linha 3
    	High4 = Temp >> 4;
    	Low4  = Temp & 0x0F;
    	LCD_CMD(High4);
    	LCD_CMD(Low4);
    ...
    if(linha == 4)
    {
    	Temp  = 0xD4 + coluna - 1;	//0xD4 � o endere�o da primeira posi��o da linha 4
    	High4 = Temp >> 4;
    	Low4  = Temp & 0x0F;
    	LCD_CMD(High4);
    	LCD_CMD(Low4);
...
//Deslocamento do texto � esquerda
void LCD_SL()
{
    LCD_CMD(0x01);
    LCD_CMD(0x08);
